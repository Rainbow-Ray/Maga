...
dayHeader = Border(
    vertical=headerGrayBorder, top=headerDashBorder, bottom=headerDashBorder
)
    ...
    for row in range(start_row + 2, start_row + 7):
        for col in range(start_column, start_column + 7):
            ws.cell(
                column=col,
                row=row,
                value=df.iloc[row - start_row - 2, col - start_column],
            )
...
def designBorder(ws):
    ...
    dayHeaderL = Border(
        vertical=headerGrayBorder,
        top=headerDashBorder,
        bottom=headerDashBorder,
    ...
    calBody = Border(
        top=calDashBorder, left=calDashBorder, right=calDashBorder, bottom=calDashBorder
    )
